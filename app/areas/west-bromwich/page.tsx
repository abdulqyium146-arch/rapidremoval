...
export async function generateMetadata(): Promise<Metadata> {
  const area = getAreaBySlug("west-bromwich");
  if (!area) return {};
  return {
    title: area.metaTitle,
    description: area.metaDescription,
    keywords: area.keywords,
    alternates: { canonical: `${SITE.url}/areas/west-bromwich` },
    openGraph: {
      title: area.metaTitle,
      description: area.metaDescription,
      url: `${SITE.url}/areas/west-bromwich`,
    },
  };
}
...
export default function AreaPage() {
  ...
  const schemas = [
    ...
    buildBreadcrumbSchema([
      ...
      { name: area.name, url: `${SITE.url}/areas/west-bromwich` },
    ]),
